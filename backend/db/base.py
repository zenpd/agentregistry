...
async def get_db() -> AsyncGenerator[AsyncSession, None]:
    """FastAPI-compatible dependency that yields a session with auto commit/rollback.

    This is a plain async generator (not @asynccontextmanager) so FastAPI can
    use it directly as a Depends() parameter. Use get_db_session() for manual
    context manager usage in orchestrations and scripts.
    """
    async with AsyncSessionLocal() as session:
        try:
            yield session
