...
async def offboard_agent(agent_id: str, user=Depends(require_update)):
    async with get_db_session() as db:
        result = await db.execute(select(Agent).where(Agent.id == agent_id))
        agent = result.scalar_one_or_none()
        if not agent:
        ...
        agent.sunset_date = date.today() + timedelta(days=90)
...
async def list_exceptions(_=Depends(require_read)):
    async with get_db_session() as db:
        result = await db.execute(
            select(GovernanceException).where(GovernanceException.expires_at > datetime.utcnow())
        )
...
async def list_discoveries(_=Depends(require_read)):
    async with get_db_session() as db:
        result = await db.execute(select(Discovery).where(Discovery.status == "pending").order_by(Discovery.confidence.desc()))
...
async def register_discovery(discovery_id: str, user=Depends(require_update)):
    async with get_db_session() as db:
        ...
        if discovery:
            ...
            discovery.resolved_at = datetime.utcnow()
